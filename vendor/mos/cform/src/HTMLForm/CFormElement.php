...
class CFormElement implements \ArrayAccess
{
    ...
    public static function create($name, $attributes) 
    {
        ...
        $types = [
            ...
            'tags' 				=> '\Mos\HTMLForm\CFormElementTags',
    ...
    public function getHTML()
    {
        ...
        $placeholder  = isset($this['placeholder']) && $this['placeholder'] ? " placeholder='" . $this['placeholder'] . (isset($this['required']) && $this['required'] ? " *" : null) . "'" : null;    
        ...
        if (in_array($this['type'], ['submit', 'reset', 'button'])) {
            ...
            return "<span><input id='$id'{$type}{$class}{$name}{$value}{$autofocus}{$readonly}{$title}{$onClick} /></span>\n";
            ...
            // textarea
			$labelAppend = "<p><label for='$id'>$label</label><br/>\n";
            return (!empty($this['label']) ? $labelAppend : "") . "<textarea id='$id'{$class}{$name}{$autofocus}{$required}{$readonly}{$placeholder}{$title}>{$onlyValue}</textarea>" . (!empty($this['label']) ? "</p>" : "") . "<p class='cf-desc'>{$description}</p>\n"; 
			...
			return "<label for='$id'>$label</label><ul id=\"tags-input\"></ul><input name=\"tags\" id=\"tags-input-values\" hidden>";
		}else {

            // Everything else 
			$labelAppend = "<p><label for='$id'>$label</label><br/>\n";
            return (!empty($this['label']) ? $labelAppend : "") . "<input id='$id'{$type}{$class}{$name}{$value}{$autofocus}{$required}{$readonly}{$placeholder}{$title}{$multiple}{$pattern}{$max}{$min}{$step}/>{$messages}" . (!empty($this['label']) ? $labelAppend : "") . "<p class='cf-desc'>{$description}</p>\n";        
